package org.sz.mbay.base.dao.utils;
...
public class PKgen {
	...
	private static PKgen pkgen = new PKgen();
	 ...
	 * 尾数通过配置设定
	 */
	private static String suffix = ResourceConfig.getProperty("cluster_number",
			"0");
	...
	public synchronized static PKgen getInstance() {
		...
		// String keyName = "pk_";
		// do {
		// suffix = MbayRandom.Digest.getRandom(2);
		// keyName += suffix;// 避免与其它重复
		// } while (ClustersContext.getAttribute(keyName) != null);
		// ClustersContext.setAttribute(keyName, true);
		// }
		return pkgen;
	}
