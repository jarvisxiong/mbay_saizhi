 ...
 */
package org.sz.mbay.base.mybatis;
...
/**
 * @Description: 本类中全部拷贝自{@code SqlSessionFactoryBean},并参考
 *               {@code ClassPathScanningCandidateComponentProvider}
 *               对部分代码做了修改，以使其满足实际需求。
 * @author han.han
 * @date 2015-3-9 下午9:56:38
 * 
 */
public class MyBatisXMLConfigBuilder extends BaseBuilder {
	...
	static final String CLASSES_PATTERN = "classes/";

	static final String TYPE_HANDLER_PACKAG = "org/sz/mbay/*/type";

	static final String BEAN_PACKAGE_PATTERN = "org/sz/mbay/.*/.*/";

	private final Pattern beanPackagePattern = Pattern
			.compile(BEAN_PACKAGE_PATTERN);

	static final String ENUM_CLASS_PATTERN = "org/sz/mbay/.*/enums/.*.class";
	// ///D:/java/apache-tomcat-7.0.56.2/wtpwebapps/mbaychannel/WEB-INF/classes/org/sz/mbay/channel/enums/package-info.class

	private final Pattern beanClassPattern = Pattern
			.compile(ENUM_CLASS_PATTERN);
	...
	private StandardServletEnvironment senvironment = new StandardServletEnvironment();
	...
	private void pluginElement(XNode parent) throws Exception {
		if (parent != null) {
			for (XNode child : parent.getChildren()) {
				...
				Interceptor interceptorInstance = (Interceptor) resolveClass(
	...
	private void objectWrapperFactoryElement(XNode context) throws Exception {
		if (context != null) {
			String type = context.getStringAttribute("type");
			ObjectWrapperFactory factory = (ObjectWrapperFactory) resolveClass(
					type).newInstance();
	...
	private void databaseIdProviderElement(XNode context) throws Exception {
		...
		if (environment != null && databaseIdProvider != null) {
			String databaseId = databaseIdProvider.getDatabaseId(environment
					.getDataSource());
			...
			for (XNode child : parent.getChildren()) {
				if ("package".equals(child.getName())) {
					...
					String packageSearchPath = ResourcePatternResolver.CLASSPATH_ALL_URL_PREFIX
							+ resolveBasePackage(typeAliasPackage);// / + "/" +
					...
					try {
						...
						for (Resource resource : resources) {
							...
							path = this.resolvePahtToPackage(path);
							if (StringUtils.isNotEmpty(path)) {
								configuration.getTypeAliasRegistry()
										.registerAliases(path);
							}
							///break;
						}
					} catch (IOException e) {
	...
	private void typeHandlerElement(XNode parent) throws Exception {
		if (parent != null) {
			for (XNode child : parent.getChildren()) {
				if ("package".equals(child.getName())) {
					String typeHandlerPackage = child
							.getStringAttribute("name");
					typeHandlerRegistry.register(typeHandlerPackage);
				} else {
					String javaTypeName = child.getStringAttribute("javaType");
					String jdbcTypeName = child.getStringAttribute("jdbcType");
					String handlerTypeName = child
	...
	private final String enumsPackage = "org/sz/mbay/**/enums";

	private void enumHandlerElement() {
		// / String typeAliasPackage = child.getStringAttribute("name");
		String packageSearchPath = ResourcePatternResolver.CLASSPATH_ALL_URL_PREFIX
				+ resolveBasePackage(enumsPackage)
				+ "/"
		...
		try {
			...
			for (Resource resource : resources) {
				String path = resource.getURL().getPath();
				path = this.resolvePathToClass(path);
				Class<?> javaTypeClass = resolveClass(path);
				// JdbcType jdbcType = resolveJdbcType(jdbcTypeName);
				Class<?> typeHandlerClass = resolveClass("org.apache.ibatis.type.EnumOrdinalTypeHandler");
				...
				if ("package".equals(child.getName())) {
					...
					if (resource != null && url == null && mapperClass == null) {
						...
						mapperParser.parse();
							...
							&& mapperClass != null) {
						Class<?> mapperInterface = Resources
	...
	protected String resolveBasePackage(String basePackage) {
		return ClassUtils.convertClassNameToResourcePath(this.senvironment
				.resolveRequiredPlaceholders(basePackage));
	}

	public String resolvePathToClass(String path) {
		Matcher matcher = beanClassPattern.matcher(path);
		if (matcher.find()) {
			path = matcher.group();
			path = path.replace("/", ".");
	...
	public String resolvePahtToPackage(String path) {
		Matcher matcher = beanPackagePattern.matcher(path);
		if (matcher.find()) {
			path = matcher.group();
			path = path.replace("/", ".");
