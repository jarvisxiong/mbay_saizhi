...
/**
 * author：han.han 此功能借鉴于strust1；
 * TokenProcessor is responsible for handling all
 * token related functionality. The methods in this class are synchronized to
 * protect token processing from multiple threads. Servlet containers are
 * allowed to return a different HttpSession object for two threads accessing
 * the same session so it is not possible to synchronize on the session.
 * 
 * @since Struts 1.1
 */
public class TokenProcessor {
	...
	private static final Logger LOGGER = LoggerFactory
			.getLogger(TokenProcessor.class);
	/**
