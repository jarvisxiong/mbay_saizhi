...
function selectmsgInfo(id){	
	$.getJSON("${pageContext.request.contextPath}/message/selectMsgInfo.mbay?id="+id,function(json){			
		if(json!=null){				
...
function deleteMsg(id){	
	$.getJSON("${pageContext.request.contextPath}/message/deleteMsg.mbay?id="+id,function(json){			
		if(json){				
			//删除成功
			$("#divIDNo"+id).hide();
			alert(json.message);
          ...
          <!--邮箱右边-->
             <div class='email_con fl'>
                <div class='sjx_middle fl'>
                 <div class='sjx_tit'>
                    <div class='sjx_tit_1 fl'>收件箱</div>                    
                 </div>
                 <div class='sjx_sec'>
                 <form id="pagerForm" action="<c:url value='msg_list.mbay'/>" method="post">
                   <!--邮箱信件-->
                   ...
                   <div class='sjx_box' onclick="selectmsgInfo(${hmsg.message.id})"  id="divIDNo${hmsg.message.id}"> 
                       <div class='sjx_box_1 clearfix'>
                        <c:if test="${hmsg.readStatus eq 1}">
                   				<div class='sjx_box_1_left sjx_box_1_u_left_sp fl'>${hmsg.senderUnumber}</div>
                  		 </c:if>   
                  		  <c:if test="${hmsg.readStatus ne 1}">
                   				<div class='sjx_box_1_left sjx_box_1_u_left fl'>${hmsg.senderUnumber}</div>
                  		 </c:if>                           
                           <a href="javascript:deleteMsg(${hmsg.message.id});"><div class='sjx_box_1_right sjx_box_1_u_right fr' title='删除'></div></a>
                       </div>
                       <div class='sjx_box_2 clearfix'>
                           <div class='sjx_box_1_left sjx_box_1_d_left fl'><fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${hmsg.message.sendTime.toDate()}"/>  </div>                           
                           <a href='<c:url value='/message/writemessage.mbay?id=${hmsg.id}' />'><div class='sjx_box_1_right sjx_box_1_d_right fr' title='回复'></div></a>
                       </div>                      
                    </div>                      
                   </c:forEach>                           
                    <div class="menea">
             ...
             <div class='sjx_detail fl'>
                  <div class='sjx_detail_u'>
                      ...
                      <div class='sj fr' id="stime"><fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${msg.sendTime.toDate()}"/></div>
                  </div>
