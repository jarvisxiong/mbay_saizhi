$(function() {
	...
	$('input[type=radio][name="traffictype"]').change(
			function() {
				/* showPackage(this.value); */
				if (hcode != null) {
					$("#trafficpackage").html("");
					queryOperatorPackage(hcode.provcode, hcode.operator,
							$(this).val());
				}
			});
	function getHcodeInfo(mobile) {
		var code = null;
		$.ajax({
			url : ctx + "/mobile/attribution.mbay?mobile=" + mobile,
			type : 'GET',
			dataType : 'json',
			async : false,
	...
	function queryOperatorPackage(area, operator, trafficType) {
		$.ajax({
			url : absurl + "/operator/queryTrafficPackage.mbay?area=" + area
					+ "&operator=" + operator + "&trafficType=" + trafficType,
			type : 'GET',
			dataType : 'json',
			async : false,
			success : function(packages) {
				if (packages != null) {
					for (var i = 0; i < packages.length; i++) {
						...
						var pack = "<span class='package ttype"
								+ tpackage.traffictype + " zhao' mbprice="
								+ tpackage.mbayprice + " value='" + tpackage.id
								+ "'>" + tpackage.traffic + "M<i></i></span>";
	...
	$('#chargemobile').keyup(
			function() {
				...
				$("#attribution").html("");
				if (reg.test(mobile)) {
					hcode = getHcodeInfo(mobile);
					if (hcode == null) {
						$("#messagenot").html("未查询到对应的手机号信息！");
						return;
					}
					$("#attribution").html(hcode.attribution);
					var trafficType = $(
							'input[type=radio][name="traffictype"]:checked')
							.val();
					$("#attribution").html(hcode.attribution);
					$("#trafficpackage").html("");// 清空流量包
					queryOperatorPackage(hcode.provcode, hcode.operator,
							trafficType);
