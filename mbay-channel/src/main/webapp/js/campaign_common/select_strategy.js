...
function showPackageSlected(obj, type, id) {
	...
	$("#yxz" + type).html("已选择：" + area + " " + operator + " " + traffic
			+ " 价格：" + mbay);
	var strhtml = "<tr id='tr" + id + "'><td>" + area + "<input p_type='"
			+ type + "' type='hidden' mbay='" + mbayprice + "' area='"
			+ areacode + "' name='packageid' value='" + id + "'/>";
...
function selectPackage() {
	...
	$(".yxz_package input[name=packageid]").each(function() {// 已选择过的
				...
				if (!ieexist) {
					...
					// 回到底部
					// var windowHeight =
					// parseInt($("body").css("height"));//整个页面的高度
					// / $("body").stop().animate({ "scrollTop": windowHeight },
					// speed);
				}
			});
...
$(function() {
			$("#subbtn").click(function() {
						var sendway = $('input[type=radio][name="sendway"]:checked');
						if (sendway.val() == 'TRAFFIC') {
							if ($("table input[name=packageid]").length == 0) {
								$("#notmsg").html("请至少选择一种策略！");
								return;
							}
							$("input[name='mbaysendunit']").val("0");
						} else if (sendway.val() == 'MBAY') {
							$("input[name='validityday']").val("0");
							//判断是否开启超出发放兑换码功能
							var checked = $('input[type=checkbox][name="continuable"]').attr("checked");
							if(checked != "checked"){
								$("input[name='quantity']").val("0");
							}
							$("#sel").attr("ignore","ignore");
						}
						$("#hd").submit();
					});
			...
			$('#tabs .yd_body .ll_box').click(function() {
				...
				$(".yxz_1").html('')
				$('.yxz_1').append(val)
						.append('<font>&nbsp;&nbsp;&nbsp;</font>').append(x_0)
						.append('<font>&nbsp;&nbsp;&nbsp;</font>').append(x_1)
						.append('<font>&nbsp;&nbsp;&nbsp;</font>')
			...
			$('.sw_sec .cl_tj').click(function() {
				...
				$('table')
						.append('<tr><td rowspan="3"></td><td rowspan="3">'
								+ isChecked_1
								+ '</td><td rowspan="3">'
								+ val
								+ '</td><td rowspan="3">'
