			...
			return {
				send : function(_, completeCallback) {
					...
					if (options.type === 'DELETE') {
						options.url = options.url + addParamChar
								+ '_method=DELETE';
						options.type = 'POST';
					...
					iframe = $('<iframe src="' + initialIframeSrc
						...
						iframe.unbind('load').bind('load', function() {
							...
							window.setTimeout(function() {
										// Removing the form in a setTimeout
										// call
										// allows Chrome's developer tools to
										// display
										// the response result
										form.remove();
									}, 0);
						...
						form.prop('target', iframe.prop('name')).prop('action',
								options.url).prop('method', options.type);
						if (options.formData) {
						...
						if (options.fileInput && options.fileInput.length
							...
							form.append(options.fileInput).prop('enctype',
									'multipart/form-data')
									// enctype must be set as encoding for IE:
									.prop('encoding', 'multipart/form-data');
							// Remove the HTML5 form attribute from the
							// input(s):
						...
						if (fileInputClones && fileInputClones.length) {
							options.fileInput.each(function(index, input) {
										...
										// Restore the original name and form
										// properties:
										$(input).prop('name',
												clone.prop('name')).attr(
												'form', clone.attr('form'));
