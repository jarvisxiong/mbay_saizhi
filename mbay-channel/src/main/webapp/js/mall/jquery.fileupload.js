...
(function(factory) {
	...
	$.support.fileInput = !(new RegExp(
			// Handle devices which give false positives for the feature
			// detection:
			'(Android (1\\.[0156]|2\\.[01]))'
	...
	$.widget('blueimp.fileupload', {
		...
		options : {
			...
			// selection, but can be disabled by setting the following option to
			// false:
			replaceFileInput : true,
			// The parameter name for the file form data (the request argument
			// name).
			// If undefined or empty, the name property of the file input field
			// is
			...
			// XHR request to keep the request size under or equal to the
			// defined
			// limit in bytes:
			limitMultiFileUploadSize : undefined,
			// Multipart file uploads add a number of bytes to each uploaded
			// file,
			// therefore the following option adds an overhead for each file
			// used
			...
			// By default, failed (abort or error) file uploads are removed from
			// the
			...
			// called once for each file in the selection for XHR file uploads,
			// else
			...
			// and allows you to override plugin options as well as define ajax
			// settings.
			//
			...
			// chunksend: function (e, data) {}, // .bind('fileuploadchunksend',
			// func);

			// Callback for successful chunk uploads:
			// chunkdone: function (e, data) {}, // .bind('fileuploadchunkdone',
			// func);

			// Callback for failed (abort or error) chunk uploads:
			// chunkfail: function (e, data) {}, // .bind('fileuploadchunkfail',
			// func);
		...
		_specialOptions : ['fileInput', 'dropZone', 'pasteZone', 'multipart',
		...
		_onProgress : function(e, data) {
			if (e.lengthComputable) {
				...
				this._progress.bitrate = this._bitrateTimer.getBitrate(now,
						this._progress.loaded, data.bitrateInterval);
				data._progress.loaded = data.loaded = loaded;
				data._progress.bitrate = data.bitrate = data._bitrateTimer
						.getBitrate(now, loaded, data.bitrateInterval);
		...
		_initProgressListener : function(options) {
			var that = this, xhr = options.xhr ? options.xhr() : $.ajaxSettings
					.xhr();
			...
			if (xhr.upload) {
				$(xhr.upload).bind('progress', function(e) {
							...
							// Make sure the progress event properties get
							// copied over:
		...
		_initXHRData : function(options) {
			...
			multipart = options.multipart || !$.support.xhrFileUpload, paramName = $
				...
				if (options.postMessage) {
					...
					if (options.blob) {
						...
						$.each(options.files, function(index, file) {
							formData.push({
								name : ($.type(options.paramName) === 'array' && options.paramName[index])
										|| paramName,
								value : file
							});
						});
					...
					if (options.blob) {
						...
						$.each(options.files, function(index, file) {
							...
							if (that._isInstanceOf('File', file)
								...
								formData
										.append(
												($.type(options.paramName) === 'array' && options.paramName[index])
														|| paramName, file,
		...
		_initIframeSettings : function(options) {
			var targetHost = $('<a></a>').prop('href', options.url)
					.prop('host');
		...
		_getParamName : function(options) {
			...
			if (!paramName) {
				...
				fileInput.each(function() {
					var input = $(this), name = input.prop('name') || 'files[]', i = (input
							.prop('files') || [1]).length;
					while (i) {
						paramName.push(name);
						i -= 1;
		...
		_initFormSettings : function(options) {
			...
			options.type = (options.type
					|| ($.type(options.form.prop('method')) === 'string' && options.form
		...
		_addConvenienceMethods : function(e, data) {
			...
			data.submit = function() {
				if (this.state() !== 'pending') {
					data.jqXHR = this.jqXHR = (that._trigger('submit', $.Event(
									'submit', {
										delegatedEvent : e
									}), this) !== false)
							&& that._onSend(e, this);
		...
		_getUploadedBytes : function(jqXHR) {
			var range = jqXHR.getResponseHeader('Range'), parts = range
					&& range.split('-'), upperBytesPos = parts
					&& parts.length > 1 && parseInt(parts[1], 10);
		...
		_chunkedUpload : function(options, testOnly) {
			...
			var that = this, file = options.files[0], fs = file.size, ub = options.uploadedBytes, mcs = options.maxChunkSize
					|| fs, slice = this._blobSlice, dfd = $.Deferred(), promise = dfd
					.promise(), jqXHR, upload;
			...
			if (ub >= fs) {
				...
				return this._getXHRPromise(false, options.context, [null,
								'error', file.error]);
			}
			...
			upload = function() {
				...
				o.contentRange = 'bytes ' + ub + '-' + (ub + o.chunkSize - 1)
						+ '/' + fs;
				...
				jqXHR = ((that._trigger('chunksend', null, o) !== false && $
						...
						.done(function(result, textStatus, jqXHR) {
							ub = that._getUploadedBytes(jqXHR)
									|| (ub + o.chunkSize);
							// Create a progress event if no final progress
							// event
							// with loaded equaling total has been triggered
							// for this chunk:
							if (currentLoaded + o.chunkSize
									- o._progress.loaded) {
							...
							if (ub < fs) {
								...
								dfd.resolveWith(o.context, [result, textStatus,
												jqXHR]);
							}
							...
							that._trigger('chunkalways', null, o);
							dfd.rejectWith(o.context, [jqXHR, textStatus,
											errorThrown]);
						});
			};
		...
		_onSend : function(e, data) {
			...
			var that = this, jqXHR, aborted, slot, pipe, options = that
					._getAJAXSettings(data), send = function() {
				that._sending += 1;
				// Set timer for bitrate progress calculation:
				options._bitrateTimer = new that._BitrateTimer();
				jqXHR = jqXHR
						|| (((aborted || that._trigger('send', $.Event('send',
										{
											delegatedEvent : e
										}), options) === false) && that
								._getXHRPromise(false, options.context, aborted))
								...
								function(jqXHR, textStatus, errorThrown) {
									that._onFail(jqXHR, textStatus,
											errorThrown, options);
								}).always(
								function(jqXHRorResult, textStatus,
										jqXHRorError) {
									that._onAlways(jqXHRorResult, textStatus,
									...
									if (options.limitConcurrentUploads
										...
										while (nextSlot) {
											if (that
													._getDeferredState(nextSlot) === 'pending') {
												nextSlot.resolve();
												break;
											}
									...
									if (that._active === 0) {
										// The stop callback is triggered when
										// all uploads have
										// been completed, equivalent to the
		...
		_onAdd : function(e, data) {
			var that = this, result = true, options = $.extend({},
					this.options, data), files = data.files, filesLength = files.length, limit = options.limitMultiFileUploads, limitSize = options.limitMultiFileUploadSize, overhead = options.limitMultiFileUploadSizeOverhead, batchSize = 0, paramName = this
					._getParamName(options), paramNameSet, paramNameSlice, fileSet, i, j = 0;
		...
		_replaceFileInput : function(input) {
			...
			// event handlers:
			this.options.fileInput = this.options.fileInput.map(
					function(i, el) {
						if (el === input[0]) {
							return inputClone[0];
						}
		...
		_getFileInputFiles : function(fileInput) {
			...
			return $.when.apply($,
					$.map(fileInput, this._getSingleFileInputFiles)).pipe(
					function() {
						return Array.prototype.concat.apply([], arguments);
					});
		},
		...
		_onPaste : function(e) {
			var items = e.originalEvent && e.originalEvent.clipboardData
					&& e.originalEvent.clipboardData.items, data = {
				files : []
			};
			if (items && items.length) {
		...
		_onDragOver : function(e) {
			...
			if (dataTransfer && $.inArray('Files', dataTransfer.types) !== -1
					&& this._trigger('dragover', $.Event('dragover', {
										delegatedEvent : e
									})) !== false) {
				e.preventDefault();
		...
		_initDataAttributes : function() {
			...
			$.each($(this.element[0].cloneNode(false)).data(), function(key,
							value) {
		...
		send : function(data) {
			if (data && !this.options.disabled) {
				if (data.fileInput && !data.files) {
					var that = this, dfd = $.Deferred(), promise = dfd
							.promise(), jqXHR, aborted;
					promise.abort = function() {
						aborted = true;
						if (jqXHR) {
							return jqXHR.abort();
