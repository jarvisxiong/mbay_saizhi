...
/**
 * 获取节点 node 节点对象(#id,.class,element) 字符类型 parent 节点的上层对象
 */
...
MECHAT.Lang = {
	telRegFatory : function(reg) {
		return [
				[ RegExp('^' + reg + '$', 'gm'),
						"<a target='_blank' href=\"tel:$1\">$1</a>" ],
				[ RegExp('^' + reg + '(\\D)', 'gm'),
						"<a target='_blank' href=\"tel:$1\">$1</a>$2" ],
				[ RegExp('(\\D)' + reg + '$', 'gm'),
						"$1<a target='_blank' href=\"tel:$2\">$2</a>" ],
				[ RegExp('(\\D)' + reg + '(\\D)', 'gm'),
						"$1<a target='_blank' href=\"tel:$2\">$2</a>$3" ] ]
	},
	regUrl : [
			[ /((ftp:\/\/)[^\u4E00-\u9FA5 ]+)/ig,
			...
			[
					/([\.A-Za-z0-9-]+?\.((com)|(net)|(im)|(cn)|(me)|(org)|(edu)|(cc)|(biz)|(info)|(tv)|(co)|(so)|(tel)|(mobi))[/?#][^\u4E00-\u9FA5， ]+)/img,
					"<a target='_blank' href=\"http://$1\">$1</a>" ],
			[
					/([\.A-Za-z0-9-]+?\.((com)|(net)|(im)|(cn)|(me)|(org)|(edu)|(cc)|(biz)|(info)|(tv)|(co)|(so)|(tel)|(mobi))[/?#])/img,
					"<a target='_blank' href=\"http://$1\">$1</a>" ],
			[
					/([\.A-Za-z0-9-]+?\.((com)|(net)|(im)|(cn)|(me)|(org)|(edu)|(cc)|(biz)|(info)|(tv)|(co)|(so)|(tel)|(mobi)))([0-9\s\u4E00-\u9FA5，]+)/img,
					"<a target='_blank' href=\"http://$1\">$1</a>$18" ],
			[
					/([\.A-Za-z0-9-]+?\.((com)|(net)|(im)|(cn)|(me)|(org)|(edu)|(cc)|(biz)|(info)|(tv)|(co)|(so)|(tel)|(mobi)))$/img,
					"<a target='_blank' href=\"http://$1\">$1</a>" ] ],
	regTel : [],

	regEmail : [ [ /([A-Z0-9\._%+-]+@[A-Z0-9\.-]+\.[A-Z]{2,15})/img,
			'<a href="mailto:$1">$1</a>' ] ]
};

MECHAT.Lang.regTel = MECHAT.Lang.regTel.concat(MECHAT.Lang
		.telRegFatory('([1]\\d{10})'));
MECHAT.Lang.regTel = MECHAT.Lang.regTel.concat(MECHAT.Lang
		.telRegFatory('([48]00\\d?-?\\d{3,4}-?\\d{3,4})'));
MECHAT.Lang.regTel = MECHAT.Lang.regTel.concat(MECHAT.Lang
		.telRegFatory('(0\\d{2,3}-?\\d{7,8})'));
MECHAT.Lang.regTel = MECHAT.Lang.regTel.concat(MECHAT.Lang
		.telRegFatory('(\\d{8})'));
...
MECHAT.Lang.isTell = function(str) {
	// var result =
	// str.match(/^(\d{2,4}[-_－—]?)?\d{3,8}([-_－—]?\d{3,8})?([-_－—]?\d{1,7})?$)|(^0?1[35]\d{9}$/);
	// var result =
	// str.match(/^[0-9]{3,4}\-[0-9]{7,8}$)|(^[0-9]{7,8}$)|(^\([0-9]{3,4}\)[0-9]{3,8}$)|(^0{0,1}13[0-9]{9}$/);
...
MECHAT.Cookie = {
	read : function(key, dc) {
		...
		var cookieStr = document.cookie, cookieValue = null;
		var start = cookieStr.indexOf(key), end = cookieStr.indexOf(";", start), end = end == -1 ? cookieStr.length
				: end;
		if (start > -1) {
	...
	set : function(key, value, expires, path, domain, secure) {
		var cookieText = encodeURIComponent(key) + "=", subCookieParts = new Array();
		switch (typeof value) {
		case 'object':
...
MECHAT.Ani.move = function(element, position, speed, callback) {
	...
	element.effect.move = setInterval(function() {
		for (var i = 0; i < parr.length; i++) {
			start[parr[i]] += (position[parr[i]] - start[parr[i]]) * speed
					/ 100;
