...
$(function() {
	...
	// 加载分享加油配置
	$.ajax({
		url: ctx + "/promotionCampaign/redeem_code/wechatConfig.mbay",
		cache: false,
		type: "GET",
		data: {
			cnumber: cnumber,
...
function share() { 
	...
	var cnumber = $("#cnumber").val();
	var code = $("#code").val();
	var playAgain = $("#playAgain").val();
	shared = false;
	if (!shareConfig.enableState || playAgain == "true") {
		$.wechat.hide();
	} else {
		$.wechat.share({
			...
			$.ajax({
				url: ctx + "/promotionCampaign/redeem_code/share_success.mbay",
				type: "GET",
				cache: false,
				data: {
					cNumber: cnumber,
				...
				success: function(resp) { 
					if (resp.status) {
						$.messager.remind({ content: "分享成功，恭喜你获取到再玩一次的机会!" });	
						shared = true; 
